...
ggplot(HD_count_sub) +
  ...
  facet_wrap(~WGS.Gene)
```
**We don't need both of the above separated graphs just wondering which graph is most beneficial with or without the totals in the background.**
...
ggplot(data = OXA_Genes, mapping = aes(x = Date.of.collection,
  ...
  facet_wrap(~ WGS.Gene)
```
The average number of OXA genes detected per month is 1.  **Could we use this graph to determine endemic levels?**
\newpage
\noindent\rule{16cm}{0.4pt}
