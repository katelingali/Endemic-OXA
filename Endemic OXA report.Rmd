...
bar <- ggplot(data = alldata)+
  geom_histogram(aes(x = Date.of.collection), by = "months", color = "#1F77B4") +
...
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
#Frequency plot of Submissions per submitter
freqplot <- ggplot(data = alldata)+
  geom_freqpoly(aes(x = Date.of.collection, group = Submitter, 
                    color = Submitter), by = "months") +
  ...
  labs(title = plottitle, subtitle = "Based on Submitter") +
  guides(x = guide_axis(angle = 45)) +
  facet_wrap(~Submitter, ncol = 4)
freqplot
```
...
bar2 <- ggplot(data = sequenced) +
  ...
  guides(fill=guide_legend(title="Gene")) #Rename legend +
...
sub_gene_table <- table1(data = alldata, ~ Submitter | WGS.Gene, overall="Total")
t1flex(sub_gene_table)
```
...
HD_map <- VA_map %>%  
   mutate("Health District" = case_when(grepl("Chesterfield", NAMELSAD) ~ "Chesterfield",
                           ...
                           grepl("Norton city", NAMELSAD) ~"Lenowisco",
...
#Rename columns
alldata_HD <- HD_count %>%
  rename("region" = "Health District", "value" = "Count")

#Joining Health Department data and map data
HD_data <- HD_group_map %>% 
  left_join(alldata_HD, by = c("Health District" = "region")) %>% 
...
map <-ggplot(merged_HD) + 
  ...
  geom_sf(fill = "transparent", color = "gray20", size = .5, 
          data = HD_group_map) + 
...
alldata_counties <- Sub_county %>%
  rename("region" = "County", "value" = "Count")

#join the county data to the VA_map
#Doesn't work for Richmond anymore because of Richmond City, Richmond Co. designation
County_data <- VA_map %>% 
...
kable(Sub_county) %>% 
  kable_classic_2()
```
Note: Isolates from Alamance County are submitted by LabCorp Burlington, but are Virginia residents.
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
#Count OXA genes per health district
alldata_HD <-alldata %>% 
  count(`Health District`, WGS.Gene) 
...
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, ft.align = "center", tab.cap = "2022 Cluster Isolates Identified Per Submitter", tab.id = "tab 18", label = "tab 18"}
Clusters4 <- Cluster_only2 %>%
  group_by(SNP.cluster, `Health District`, Submitter) 
Clusters4
#Genes per Cluster table
cluster_data <- table1(~ `Health District` + Submitter | SNP.cluster, data=alldata)
cluster_data
table_18 <- t1flex(cluster_2022)
autofit(table_18)
```
...
```
\newpage
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, ft.align = "center", tab.cap = "2020 OXA Genes Identified Per Cluster", tab.id = "tab 3", label = "tab 3"}
#Set table settings
set_flextable_defaults(
  font.family = "Roboto",
  font.size = 7, 
...
#Genes per cluster table
cluster_2020 <- table1(data = crab_2020, ~SNP.cluster | WGS.Gene, overall="Total")
cluster_tab <- t1flex(cluster_2020)
cluster_tab
```
\newpage
\noindent\rule{16cm}{0.4pt}
...
```
\newpage
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, ft.align = "center", tab.cap = "2021 OXA Genes Identified Per Cluster", tab.id = "tab 4", label = "tab 4"}
#Set table settings
set_flextable_defaults(
  font.family = "Roboto",
...
#Genes per cluster table
cluster_2021 <- table1(data = crab_2021, ~ SNP.cluster | WGS.Gene, overall="Total")
t1flex(cluster_2021)
```
\newpage
\noindent\rule{16cm}{0.4pt}
...
```
\newpage
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, ft.align = "center", tab.cap = "2022 OXA Genes Identified Per Cluster", tab.id = "tab 5", label = "tab 5"}
#Genes per Cluster table
cluster_2022 <- table1( ~ SNP.cluster | WGS.Gene, data = crab_2022,  overall="Total")
table_5 <- t1flex(cluster_2022)
table_5
```
