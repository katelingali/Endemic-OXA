...
#choose SDE_USDC_CENSUS_VA_COUNTY.shp in downloads or SP                                                                                                                                
VA_map <- file.choose()
```
...
crab_2020 %>% 
  ...
  ggplot() +
  geom_col(mapping = aes(y = Submitter, fill = Cluster.ID, x = n), 
           color = "black") +
  ...
  legend.key.size = unit(.5, 'cm'),
        legend.text=element_text(size=9)) +
  guides(fill=guide_legend(title="Cluster ID")) #Rename legend
```
Cluster IDs have been color coded to assist in identifying clusters from one year to the next in this and the following graphs of yearly data.
...
crab_2021 %>% 
  count(Submitter, Cluster.ID) %>% 
  ggplot() +
  geom_col(mapping = aes(y = Submitter, fill = Cluster.ID, x = n), color = "black") +
  scale_fill_manual(values=Cluster_Palette) +
  scale_x_continuous(expand = c(0,0)) +
  xlab("Number of Isolates") +
...
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, ft.align = "left", tab.cap = "2023 OXA Genes Identified Per Cluster", tab.id = "tab 10", label = "tab 10"}
#Genes per Cluster table
crab_2023 %>% 
  select(Cluster.ID, WGS.Gene) %>% 
  tbl_summary(by = WGS.Gene) %>%
